...
public class Mecanico {
	...
	public boolean asignarVehiculo(Vehiculo vehiculoAsignado, Mecanico mecanicoAsignado) {
		TallerMecanico taller = new TallerMecanico();
		ArrayList<Vehiculo> vehiculos = taller.getVehiculos();
		ArrayList<Mecanico> mecanicos = taller.getMecanicos();
		
		for (Vehiculo vehiculoBuscado : vehiculos ) {
			if (vehiculoBuscado.getMatricula()==vehiculoAsignado.getMatricula()) {
				for (Mecanico mecanicoBuscado: mecanicos) {
					if (mecanicoBuscado.estadoLibreMecanico==mecanicoAsignado.estadoLibreMecanico){ 
						//&&mecanicoBuscado.estadoLibreMecanico == true {
	...
	public boolean cambiarEstadoFalse() {
		return estadoLibreMecanico = false;
